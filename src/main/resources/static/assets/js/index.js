...
layui.config({
	...
	//请先部署到 localhost 或 web 服务器下再访问
	if(! new RegExp(/^http(s*):\/\//).test(window.location.href)){
		layer.alert("请先部署到 localhost 或 web 服务器下再访问！", {"title": "部署提示"});
	}
	
	//渲染导航菜单（出现左侧垂直导航）
	// var requestNavUrl = "assets/json/navs.json";	//请求导航菜单数据的接口地址（这里 json 是相对于 index.html 的路径）
	var requestNavUrl = "showLeftTrees";
	navTree.render({
